...
def add_bot(message):
    ...
    try:
        ...
        bot_master.reply_to(message, f"✅ *Token aceptado*. El bot *{escape_markdown(bot_name)}* está conectado.")
        ...
        error_message = f"❌ *Token inválido*: {escape_markdown(str(e))}"
...
def list_bots(message):
    if connected_bots:
        bot_list = '\n'.join([f"- @{escape_markdown(name)}" for name in connected_bots.keys()])
